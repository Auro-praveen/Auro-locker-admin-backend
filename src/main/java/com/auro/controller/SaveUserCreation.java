...
//				String allPerm = jsonObj.getString("allPermissions");
				...
				List<User> isUserExist = (List<User>) session.createQuery(hql)
						.setParameter("uName", jsonObj.get("userName").toString()).getResultList();
...
//						user.setApp_access_allowed("");
			...
			try {
				appPermissions = jsonObj.getString("appPermissions");
			} catch (JSONException e) {
				// TODO: handle exception
				appPermissions = null;
			}
